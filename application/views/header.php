...
if($this->session->userdata('unique_id')==''){
	$this->session->set_userdata('unique_id', rand());
}
?>
...
<!-- header -->
	<div class="agileits_header">
		...
		<div class="product_list_header">  
			<form action="<?=base_url()?>mycart" method="post" class="last" >
                <fieldset>
                    <input type="hidden" name="cmd" value="_cart" />
                    <input type="hidden" name="display" value="1" />
                    <input type="submit" name="submit" value="checkout : <?=$this->session->userdata('cart_count')?>"

                    class="button fa fa-cart-plus"  />
                </fieldset>
            </form>
		</div>
